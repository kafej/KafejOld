...
var m3D = function () { 
	...
	Diapo.prototype.anim = function () { 
		if (this.isLoaded) { 
			...
			this.css.top    = Math.round(nh + y * p - h * .6) + 'px'; 
		...
		/* ---- z axis move ---- */ 
			...
			if (!m) { 
				...
				if (selected.url) { 
					...
					urlInfo.style.top = Math.round(selected.img.offsetTop + selected.img.offsetHeight - urlInfo.offsetHeight - 6) + "px"; 
